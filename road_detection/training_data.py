...
def generate_rgb_one_hot(road_file, image_file, meta_file, out_dir,
                         img_buff=128, num_images=5000,
                         color_scales=[2000.0, 2800.0, 2000.0], img_scale=1.0):
    ...
    for ix, road_ind in enumerate(road_inds):
        ...
        raw_chip[:, :, 2] = misc.imresize(images[0][ys:ye, xs:xe].copy(),
                                          img_scale)
        raw_file = os.path.join(out_dir, 'true_' +
                                str(ix).zfill(len(str(num_images))) + '.png')
        misc.imsave(raw_file, raw_chip)

    xs = np.arange(img_buff, x_size - img_buff - 1)
    ys = np.arange(img_buff, y_size - img_buff - 1)

    xinds = np.random.choice(xs, num_images * 2, replace=True)
    yinds = np.random.choice(ys, num_images * 2, replace=True)
    ...
    for x, y in zip(xinds, yinds):
        ...
        raw_chip[:, :, 2] = misc.imresize(images[0][ys:ye, xs:xe].copy(),
                                          img_scale)
        raw_file = os.path.join(out_dir, 'false_' +
                                str(false_counter).zfill(len(str(
                                    num_images))) + '.png')
        false_counter += 1
        misc.imsave(raw_file, raw_chip)
        if false_counter >= (num_images - 1):
