...
for dirpath, dirnames, filenames in os.walk(in_dir):
    for filename in [f for f in filenames
        ...
        truth_files.append(os.path.join(dirpath, filename))
...
train_files = np.array(raw_files)[train_inds].tolist()
train_targets = np.array(truth_files)[train_inds].tolist()
test_files = np.array(raw_files)[test_inds].tolist()
test_targets = np.array(truth_files)[test_inds].tolist()

# Load in the train images
images_train = np.zeros((len(train_files), img_size, img_size, 1),
                        dtype='uint8')
for ix, f in enumerate(train_files):
    images_train[ix, :, :, 0] = misc.imresize(misc.imread(f), (img_size,
                                                               img_size))

# Load in the train truth
truth_train = np.zeros((len(train_targets), truth_size * truth_size),
                       dtype='uint8')
for ix, f in enumerate(train_targets):
    truth_train[ix, :] = misc.imresize(misc.imread(f),
                                       (truth_size, truth_size)).flatten()

# Load in the test images
images_test = np.zeros((len(test_files), img_size, img_size, 1),
                       dtype='uint8')
for ix, f in enumerate(test_files):
    images_test[ix, :, :, 0] = misc.imresize(misc.imread(f),
                                             (img_size, img_size))

# Load in the test truth
truth_test = np.zeros((len(test_targets), truth_size * truth_size),
                      dtype='uint8')
...
num_classes = truth_size * truth_size

''' Set up the CNN parameters for the network '''
...
def main_network(images, training):
    ...
    # with pt.defaults_scope(activation_fn=tf.nn.relu, phase=phase):
    #     y_pred, loss = x_pretty.\
    #         conv2d(kernel=filter_size1, depth=num_filters1, name='layer_conv1', batch_normalize=True).\
    #         max_pool(kernel=2, stride=2).\
...
def predict_cls(images, labels, cls_true):
    ...
    cls_pred = np.zeros(shape=(num_images, img_size * img_size), dtype=np.int)
